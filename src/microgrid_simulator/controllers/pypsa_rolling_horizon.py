"""PyPSARollingHorizonController — rolling-horizon optimal dispatch baseline.

Every ``rolling_horizon_hours`` the controller re-solves a PyPSA
unit-commitment/dispatch over the next ``horizon_hours`` using a validated
cached or live forecast, then plays back the plan until the next replan. This
is the rolling-horizon optimization baseline used for comparison with RL.

Requires the ``ops`` extra and a
:class:`~microgrid_simulator.backends.pypsa_backend.PyPSAOperationalBackend`
for its optimization model. Realized actions are executed by the separately
configured environment plant.
"""
...
SnapshotProvider = Callable[
    [], ForecastSnapshot | tuple[str | None, ForecastSnapshot | None] | None
]
...
def _align_snapshot_to_steps(
    ...
    *,
    ...
    ``pad_incomplete`` is set (retained for the legacy hourly F0
    ...
    if (
        ...
        raise ForecastError(
            f"forecast source {snapshot.source_id!r} does not match expected {expected_source_id!r}"
        )
    ...
    if issued_snap > decision:
        raise ForecastError(
            ...
            "refusing for leakage-free PyPSA-RH replay"
        )
    ...
    if covered_ticks < n_steps:
        if pad_incomplete:
            ...
            raise ForecastError(
                ...
                f"control ticks but PyPSA-RH needs {n_steps}; refusing an incomplete horizon"
            )
...
class PyPSARollingHorizonController(Controller):
    name = "pypsa_rh"
    ...
    def __init__(
        ...
        if backend is None:
            raise ValueError(
                "PyPSARollingHorizonController needs a PyPSAOperationalBackend instance"
            )
        ...
        # Cached-forecast PyPSA-RH: when a cache is given the controller re-solves
        ...
        # Live-forecast PyPSA-RH: when a snapshot callback is supplied the controller
    ...
    def act(self, state: GridState) -> ControlAction:
        if not self._actions or self._since_replan >= self.replan_every:
            ...
            if self._forecast_cache is not None:
                ...
                raise ForecastError(
                    "PyPSA rolling-horizon needs a cached forecast or a live forecast snapshot; "
    ...
    def _resolve_live_snapshot(self, state: GridState) -> tuple[np.ndarray, np.ndarray]:
        if self._snapshot_provider is None:
            raise ForecastError("no live forecast snapshot provider is configured for PyPSA-RH ")
        ...
        if snapshot is None:
            raise ForecastError(
                "no live forecast snapshot is available for PyPSA-RH at this replan; "
                "failing closed instead of using perfect foresight"
            )
        if not isinstance(snapshot, ForecastSnapshot):
            raise TypeError("live forecast provider must return a ForecastSnapshot or None")
