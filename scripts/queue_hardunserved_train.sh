...
# scenario (3 seeds x 1M steps), one at a time to avoid CPU overheating.
set -u
...
STEPS=1000000
ARTIFACT_DIR="artifacts/sac/hardunserved-1m"
SEEDS=(0 1 2)

echo "state=starting" > logs/queue_state.txt
echo "=== $(date) watcher started (hard_unserved=true) ===" >> logs/queue_run.log
...
for seed in "${SEEDS[@]}"; do
    ...
    nohup uv run microgrid-sim train \
        ...
        > "logs/train_hardunserved_seed${seed}.log" 2>&1 &
    TRAIN_PID=$!
