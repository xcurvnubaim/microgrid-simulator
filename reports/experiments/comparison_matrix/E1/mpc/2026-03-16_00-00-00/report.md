...
| Served load energy | 6,681.891 | kWh | model output |
| Load served | 86.453 | % | headline reliability |
| Unserved energy | 1,047.009 | kWh | headline failure metric |
| Blackout/brownout duration | 29.750 | h | 15-min intervals with unserved load |
| Peak unserved power | 105.203 | kW | maximum interval |
| Measured PV available | 3,363.202 | kWh | negative standby clipped to zero |
| PV used | 3,312.105 | kWh | before source attribution |
| PV curtailed/spilled | 51.096 | kWh | PV-only waste definition |
| Diesel generation | 3,473.713 | kWh | assumed generator model |
| Diesel load-serving energy | 2,814.013 | kWh | PV-first accounting attribution |
| Diesel overgeneration | 190.231 | kWh | diesel share routed to the dump-load path |
| Diesel useful output | 94.524 | % | generation not classified as dumped diesel excess |
| Diesel runtime | 39.500 | h | realized on-state |
| Diesel starts | 6.000 | count | realized starts |
| Diesel carbon proxy | 2,431.599 | kg CO2e | 0.70 kg/kWh assumed |
| Battery charge energy | 659.807 | kWh | AC-side terminal energy |
...
| Battery charge from diesel | 469.469 | kWh | surplus-diesel share of charging |
| Battery discharge energy | 746.111 | kWh | AC-side terminal energy |
| Battery throughput | 1,405.919 | kWh | charge + discharge |
| Final SOC | 21.237 | % | assumed 500 kWh battery |
| SoH loss proxy | 0.057 | percentage points | placeholder degradation law |
| Excess generation | 190.231 | kWh | non-load-serving generation |
| Dump-load energy | 190.231 | kWh | explicit sink for non-exportable surplus |
| Network losses | 97.793 | kWh | separate from excess generation |
...
|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 2026-03-16 | 2,400.100 | 89.495 | 252.138 | 1,191.674 | 1,019.245 | 544.801 | 713.472 | 38.891 |
| 2 | 2026-03-17 | 2,754.000 | 80.096 | 548.155 | 973.051 | 1,235.568 | 424.681 | 864.897 | 34.878 |
| 3 | 2026-03-18 | 2,574.800 | 90.418 | 246.717 | 1,147.379 | 1,218.900 | 436.436 | 853.230 | 21.237 |
...
| `penalty_carbon` | 9,726.396780 |
| `penalty_constraint` | 28.762678 |
| `penalty_excess` | 190.231428 |
| `penalty_fuel` | 20,277.823051 |
| `penalty_health` | 2.850485 |
| `penalty_unserved` | 20,940.179980 |
| `penalty_waste` | 51.096238 |
...
```bash
uv run microgrid-sim replay --config /home/ma012/projects/microgrid-simulator/configs/economic-e1-high-diesel-fuel.yaml --output-dir /home/ma012/projects/microgrid-simulator/reports/experiments/comparison_matrix/E1/mpc/2026-03-16_00-00-00 --policy mpc
```
