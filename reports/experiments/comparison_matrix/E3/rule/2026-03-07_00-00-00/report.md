...
| Served load energy | 6,525.934 | kWh | model output |
| Load served | 98.500 | % | headline reliability |
| Unserved energy | 99.366 | kWh | headline failure metric |
| Blackout/brownout duration | 9.250 | h | 15-min intervals with unserved load |
| Peak unserved power | 120.693 | kW | maximum interval |
| Measured PV available | 895.268 | kWh | negative standby clipped to zero |
| PV used | 368.012 | kWh | before source attribution |
| PV curtailed/spilled | 527.255 | kWh | PV-only waste definition |
| Diesel generation | 7,220.889 | kWh | assumed generator model |
| Diesel load-serving energy | 6,163.986 | kWh | PV-first accounting attribution |
| Diesel overgeneration | 1,050.698 | kWh | diesel share routed to the dump-load path |
| Diesel useful output | 85.449 | % | generation not classified as dumped diesel excess |
| Diesel runtime | 71.000 | h | realized on-state |
| Diesel starts | 3.000 | count | realized starts |
| Diesel carbon proxy | 5,054.622 | kg CO2e | 0.70 kg/kWh assumed |
...
| Battery throughput | 12.269 | kWh | charge + discharge |
| Final SOC | 52.356 | % | assumed 500 kWh battery |
| SoH loss proxy | 0.000 | percentage points | placeholder degradation law |
| Excess generation | 1,050.698 | kWh | non-load-serving generation |
| Dump-load energy | 1,050.698 | kWh | explicit sink for non-exportable surplus |
| Network losses | 70.823 | kWh | separate from excess generation |
...
|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 2026-03-07 | 2,788.500 | 98.002 | 55.707 | 183.472 | 2,854.582 | 0.000 | 1,998.207 | 50.000 |
| 2 | 2026-03-08 | 1,503.900 | 99.900 | 1.504 | 86.764 | 1,839.341 | 12.269 | 1,287.538 | 52.356 |
| 3 | 2026-03-09 | 2,332.900 | 98.193 | 42.155 | 97.776 | 2,526.966 | 0.000 | 1,768.876 | 52.356 |
...
| `penalty_carbon` | 20,218.488493 |
| `penalty_constraint` | 2.355583 |
| `penalty_excess` | 1,050.698076 |
| `penalty_fuel` | 36,952.265989 |
| `penalty_health` | 0.004907 |
| `penalty_unserved` | 1,987.311326 |
| `penalty_waste` | 527.255371 |
...
```bash
uv run microgrid-sim replay --config /home/ma012/projects/microgrid-simulator/configs/economic-e3-high-fuel-low-battery-use.yaml --output-dir /home/ma012/projects/microgrid-simulator/reports/experiments/comparison_matrix/E3/rule/2026-03-07_00-00-00 --policy rule
```
