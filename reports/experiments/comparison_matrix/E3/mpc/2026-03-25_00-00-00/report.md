...
| Served load energy | 7,173.908 | kWh | model output |
| Load served | 90.370 | % | headline reliability |
| Unserved energy | 764.492 | kWh | headline failure metric |
| Blackout/brownout duration | 28.750 | h | 15-min intervals with unserved load |
| Peak unserved power | 118.944 | kW | maximum interval |
| Measured PV available | 1,484.190 | kWh | negative standby clipped to zero |
| PV used | 1,145.969 | kWh | before source attribution |
| PV curtailed/spilled | 338.221 | kWh | PV-only waste definition |
| Diesel generation | 6,263.681 | kWh | assumed generator model |
| Diesel load-serving energy | 5,474.967 | kWh | PV-first accounting attribution |
| Diesel overgeneration | 255.439 | kWh | diesel share routed to the dump-load path |
| Diesel useful output | 95.922 | % | generation not classified as dumped diesel excess |
| Diesel runtime | 60.250 | h | realized on-state |
| Diesel starts | 8.000 | count | realized starts |
| Diesel carbon proxy | 4,384.577 | kg CO2e | 0.70 kg/kWh assumed |
| Battery charge energy | 533.275 | kWh | AC-side terminal energy |
...
| Battery charge from diesel | 533.275 | kWh | surplus-diesel share of charging |
| Battery discharge energy | 552.972 | kWh | AC-side terminal energy |
| Battery throughput | 1,086.247 | kWh | charge + discharge |
| Final SOC | 37.183 | % | assumed 500 kWh battery |
| SoH loss proxy | 0.043 | percentage points | placeholder degradation law |
| Excess generation | 255.439 | kWh | non-load-serving generation |
| Dump-load energy | 255.439 | kWh | explicit sink for non-exportable surplus |
| Network losses | 88.340 | kWh | separate from excess generation |
...
|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 2026-03-25 | 2,553.700 | 90.757 | 236.045 | 644.970 | 1,750.622 | 440.868 | 1,225.435 | 38.674 |
| 2 | 2026-03-26 | 2,885.200 | 85.408 | 421.013 | 411.808 | 2,146.014 | 302.716 | 1,502.210 | 52.021 |
| 3 | 2026-03-27 | 2,499.500 | 95.702 | 107.434 | 89.191 | 2,367.045 | 342.663 | 1,656.932 | 37.183 |
...
| `penalty_carbon` | 17,538.307892 |
| `penalty_constraint` | 12.817174 |
| `penalty_excess` | 255.439454 |
| `penalty_fuel` | 36,873.670671 |
| `penalty_health` | 0.434499 |
| `penalty_unserved` | 15,289.847461 |
| `penalty_waste` | 338.221163 |
...
```bash
uv run microgrid-sim replay --config /home/ma012/projects/microgrid-simulator/configs/economic-e3-high-fuel-low-battery-use.yaml --output-dir /home/ma012/projects/microgrid-simulator/reports/experiments/comparison_matrix/E3/mpc/2026-03-25_00-00-00 --policy mpc
```
